...
public class Base64Util {
    ...
    public static String compressData(byte[] data) {  
        try {  
        	log.info("压缩前文件大小data===>" + data.length);
            ByteArrayOutputStream bos = new ByteArrayOutputStream();  
            DeflaterOutputStream zos = new DeflaterOutputStream(bos);  
            zos.write(data);  
            zos.close();  
            log.info("压缩后文件大小bos.toByteArray()===>" + bos.toByteArray().length);
            return byteToBase64Encoding(bos.toByteArray()); //先压缩，在转码
        } catch (Exception ex) {  
    ...
    public static byte[] decompressData(String encdata) {
        try {
        	log.info("接收解码前大小===>" + encdata.getBytes().length);
        	byte[] byteToBaseData = byteToBase64Decoding(encdata);
        	log.info("Base64解码后大小===>" + byteToBaseData.length);
        	if(byteToBaseData != null){
	            ByteArrayOutputStream bos = new ByteArrayOutputStream();
	            InflaterOutputStream zos = new InflaterOutputStream(bos);
	            zos.write(byteToBaseData);
	            zos.close();
	            log.info("解压后大小===>" + bos.toByteArray().length);
	...
	public static void main(String[] args) throws IOException {
		File file = new File("D:\\IMG_5089.JPG");
		byte[] fileByte = getFileByte(file);

		String result = compressData(fileByte);
//		log.info("解码===>" + result);
