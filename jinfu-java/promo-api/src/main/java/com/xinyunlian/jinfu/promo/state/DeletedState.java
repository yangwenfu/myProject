 ...
 */
public class DeletedState implements PromotionState {
    @Override
    public void on(PromoInfDto promoInfDto) {
        if(promoInfDto.getStatus().equals(EPromoInfStatus.INVALID)){
            promoInfDto.setStatus(EPromoInfStatus.DELETED);
            return;
        }
